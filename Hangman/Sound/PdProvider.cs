...
namespace HangmanFun.Sound
{
    class PdProvider : IWaveProvider, IDisposable
    {
        ...
        void SetUpBuffer()
        {
            ...
            _minBuffer = blocksize * Ticks * _pd.Outputs * 2;
        }
        ...
        void SetUpPd()
        {
            ...
            // Open Pd patch
            _patch = _pd.LoadPatch(@".\Assets\test.pd");
        ...
        void RefillBuffer()
        {
            if (_pd == null || _patch == null)
                throw new InvalidOperationException("Pd or Patch not initialized.");

            int blockSize = _pd.BlockSize;
            if (blockSize <= 0)
                throw new InvalidOperationException("Pd BlockSize is invalid.");

            while (_circularBuffer.Count < _minBuffer)
            {
                float[] inputBuffer = new float[_pd.Inputs * blockSize * Ticks]; // fill with zeros if no input
                // Compute audio. Take care of the array sizes for audio in and out.z
                bool success = _pd.Process(Ticks, inputBuffer, _pdBuffer);
                if (!success)
                    throw new InvalidOperationException("Pd.Process failed.");
